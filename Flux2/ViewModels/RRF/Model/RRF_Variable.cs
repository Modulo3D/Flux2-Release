...
{
    public class RRF_VariableObjectModel<TModel, TRData, TWData> : FLUX_ObservableVariable<RRF_ConnectionProvider, RRF_Connection, TRData, TWData>
    {
        ...
        static IObservable<Optional<TRData>> observe_func(
            RRF_Connection connection,
            Func<RRF_ObjectModel, IObservable<Optional<TModel>>> get_model, 
            Func<RRF_Connection, TModel, Optional<TRData>> get_data)
        {
            return connection.MemoryBuffer.ObserveModel(get_model, get_data);
        }
        static IObservable<Optional<TRData>> observe_func(
            RRF_Connection connection,
            Func<RRF_ObjectModel, IObservable<Optional<TModel>>> get_model,
        ...
        {
            var model = await read_model(connection.MemoryBuffer);
            if (!model.HasValue)
                return default;
    ...
    public class RRF_VariableGlobalModel<TData> : FLUX_ObservableVariable<RRF_ConnectionProvider, RRF_Connection, TData, TData>, IRRF_VariableGlobalModel
    {
        ...
        {
            return connection.MemoryBuffer.ObserveGlobalModel(m => get_data(m, variable, convert_data));
        }
