...
namespace Flux.ViewModels
{
    public class MCodeQueueViewModel : RemoteControl<MCodeQueueViewModel>, IFluxMCodeQueueViewModel
    {
        ...
        [RemoteOutput(true)]
        public short QueuePosition => FluxJob.QueuePosition;

        [RemoteCommand]
        public ReactiveCommandBaseRC<Unit, Unit> DeleteMCodeQueueCommand { get; }

        [RemoteCommand]
        public ReactiveCommandBaseRC<Unit, Unit> MoveUpMCodeQueueCommand { get; }

        [RemoteCommand]
        public ReactiveCommandBaseRC<Unit, Unit> MoveDownMCodeQueueCommand { get; }
        ...
        public MCodeQueueViewModel(FluxViewModel flux, MCodesViewModel mcodes, FluxJob job)
            : base($"{typeof(MCodeQueueViewModel).GetRemoteElementClass()};{job.QueuePosition}")
        {
            FluxJob = job;
            MCodes = mcodes;

            var queue_position = MCodes.Flux.ConnectionProvider
            ...
            var can_move_up = Observable.CombineLatest(is_idle, queue_position, CanMoveUpQueue);
            var can_delete = Observable.CombineLatest(can_safe_cycle, queue_position, CanDeleteQueue);
            var can_move_down = Observable.CombineLatest(is_idle, queue_position, last_queue_pos, CanMoveDownQueue);
