...
namespace Flux.ViewModels
{
    ...
    {
        public const int MessagePageSize = 15;


        private readonly ObservableAsPropertyHelper<int> _MessagePageCount;
        public int MessagePageCount => _MessagePageCount.Value;
        ...
        private PageRequest _MessagePage = new PageRequest(1, MessagePageSize);
        public PageRequest MessagePage
        {
            get => _MessagePage;
            set => this.RaiseAndSetIfChanged(ref _MessagePage, value);
        }

        [RemoteOutput(true)]
        public IObservableList<Message> PagedMessages { get; private set; }
        [RemoteCommand]
        ...
        public IFLUX_ConnectionProvider ConnectionProvider { get; }

        protected ObservableAsPropertyHelper<MessageFlags> _MessageFlags;
        public MessageFlags MessageFlags => _MessageFlags.Value;

        public MessagesViewModel(FluxViewModel flux) : base(flux)
        {
            ConnectionProvider = flux.ConnectionProvider;

            var messages = this.WhenAnyValue(m => m.CurrentTimestamp)
                .StartWith(DateTime.Now)
                .SelectAsync(async state =>
                {
                    var message_list = new List<Message>();
                    //for (int i = 0; i < 1000; i++)
                    //{
                    //    message_list.Add(new Message(MessageLevel.DEBUG, DateTime.Now, $"{i}", "testing many errors..."));
                    //}

                    var messages = await ConnectionProvider.ReadVariableAsync(c => c.MESSAGES);
                    if (!messages.HasValue)
                        return message_list;

                    var read_files_cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
                    var debug_messages = await ReadMessages(messages.Value, MessageLevel.DEBUG, read_files_cts.Token);
                    var info_messages = await ReadMessages(messages.Value, MessageLevel.INFO, read_files_cts.Token);
                    var warning_messages = await ReadMessages(messages.Value, MessageLevel.WARNING, read_files_cts.Token);
                    var error_messages = await ReadMessages(messages.Value, MessageLevel.ERROR, read_files_cts.Token);
                    var emergency_messages = await ReadMessages(messages.Value, MessageLevel.EMERG, read_files_cts.Token);

                    if (debug_messages.HasValue)
                        message_list.Add(debug_messages.Value);
                    if (info_messages.HasValue)
                        message_list.Add(info_messages.Value);
                    if (warning_messages.HasValue)
                        message_list.Add(warning_messages.Value);
                    if (error_messages.HasValue)
                        message_list.Add(error_messages.Value);
                    if (emergency_messages.HasValue)
                        message_list.Add(emergency_messages.Value);
            ...
            _MessagePageCount = messages.Connect().Count()
                .Select(count => (int)Math.Ceiling(Math.Max(1, count / (float)MessagePageSize)))
                .ToPropertyRC(this, v => v.MessagePageCount);

            _Page = Observable.CombineLatest(
                this.WhenAnyValue(v => v.MessagePage),
                this.WhenAnyValue(v => v.MessagePageCount),
                (page_request, page_count) =>
                $"{page_request.Page}/{page_count}")
                .ToPropertyRC(this, v => v.Page);

            var page_request = this.WhenAnyValue(v => v.MessagePage);
            var message_sort = SortExpressionComparer<Message>
                .Descending(v => v.Timestamp);

            PagedMessages = messages.Connect()
                .Sort(message_sort)
                .Page(page_request)
                .AsObservableListRC(this);
            ...
            var can_previous_page_command = this.WhenAnyValue(v => v.MessagePage).Select(page_request => page_request.Page > 1);

            NextPageCommand = ReactiveCommandBaseRC.Create(() => { MessagePage = new PageRequest(MessagePage.Page + 1, MessagePageSize); }, this, can_next_page_command);
            PreviousPageCommand = ReactiveCommandBaseRC.Create(() => { MessagePage = new PageRequest(MessagePage.Page - 1, MessagePageSize); }, this, can_previous_page_command);
        }

        public void Initialize()
        {
            _MessageFlags = Observable.CombineLatest(
                ConnectionProvider.ObserveVariable(c => c.MESSAGES),
                this.WhenAnyValue(m => m.CurrentTimestamp),
                (messages, current_timestamp) =>
                {
                    if (!messages.HasValue)
                        return default;

                    var has_debug = HasMessage(messages.Value, MessageLevel.DEBUG, current_timestamp);
                    var has_info = HasMessage(messages.Value, MessageLevel.INFO, current_timestamp);
                    var has_warning = HasMessage(messages.Value, MessageLevel.WARNING, current_timestamp);
                    var has_error = HasMessage(messages.Value, MessageLevel.ERROR, current_timestamp);
                    var has_emerg = HasMessage(messages.Value, MessageLevel.EMERG, current_timestamp);

                    return new MessageFlags(has_debug, has_info, has_warning, has_error, has_emerg);
                })
                .ToPropertyRC(this, m => m.MessageFlags);
        }
        private bool HasMessage(FLUX_FileList messages, MessageLevel level, DateTime current_timespan)
        {
            var filename = level switch
            {
                ...
                MessageLevel.EMERG => "emergency",
                _ => "",
            };
            return messages.Files
                .FirstOrOptional(m => m.Name == filename)
                .Convert(m => m.Date)
                .Convert(date => date > current_timespan)
                .ValueOr(() => false);
        }

        private async Task<Optional<List<Message>>> ReadMessages(FLUX_FileList fileList, MessageLevel level, CancellationToken ct)
        {
            var filename = level switch
            {
                MessageLevel.DEBUG => "debug",
            ...
            var message_file_payload = await fileList.Files
                .FirstOrOptional(f => f.Name == filename)
                .ConvertAsync(file => ConnectionProvider.GetFileAsync(c => c.MessageEventPath, file.Name, ct));
            if (!message_file_payload.HasValue)
                return Optional<List<Message>>.None;
            return message_file_payload.Value.ReadLines()
                .Select(message => ConnectionProvider.ParseMessage(level, message))
                .Where(m => m.HasValue)
                .Select(m => m.Value)
                .ToList();
        }
        ...
        //public void LogMessage(FluxMessage message)
        //{
        ...
        //}
        //public void LogMessage(string message, MessageLevel level, DateTime timestamp, int errCode)
        //{
        //    LogMessage(new FluxMessage(level, timestamp, errCode, message));
        //}
