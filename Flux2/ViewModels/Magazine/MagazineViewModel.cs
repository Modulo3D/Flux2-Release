...
namespace Flux.ViewModels
{
    public class MagazineViewModel : FluxRoutableViewModel<MagazineViewModel>
    {
        ...
        [RemoteCommand]
        public Optional<ReactiveCommandBaseRC> ResetMagazineCommand { get; internal set; }
        ...
        public MagazineViewModel(FluxViewModel flux) : base(flux)
        {
            ...
            if (Flux.ConnectionProvider.VariableStoreBase.HasToolChange)
                ResetMagazineCommand = ReactiveCommandBaseRC.CreateFromTask(Flux.ConnectionProvider.ResetMagazineAsync, this, is_idle);
            ...
            if (Flux.ConnectionProvider.HasVariable(m => m.LOCK_CLOSED, top_lock_unit))
                AddCommand("toggleTopLock", ReactiveCommandBaseRC.CreateFromTask(async () => { await Flux.ConnectionProvider.ToggleVariableAsync(c => c.OPEN_LOCK, top_lock_unit); }, this));
        }
    }
}
